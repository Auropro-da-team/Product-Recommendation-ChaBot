...
import { sendMessageToChatbot, startNewConversation, fetchChatHistory, getSessionId, getLocalChatHistory, saveChatHistory } from "../api";
...
function Chatbot({ selectedChat, onSendMessage, updateChatHistory, chatHistory = [], loading }) {
  const [messages, setMessages] = useImmer(() => {
    ...
    if (storedConversationId) {
      ...
      const storedChat = localHistory.find((chat) => chat.conversation_id === storedConversationId);
  ...
  const [conversationId, setConversationId] = useState(
    selectedChat?.conversation_id || localStorage.getItem("conversation_id") || null
  );
  ...
  useEffect(() => {
    const loadChat = async () => {
      ...
      if (selectedChat?.conversation_id && currentConversationId !== selectedChat.conversation_id) {
        currentConversationId = selectedChat.conversation_id;
        setConversationId(currentConversationId);
        localStorage.setItem("conversation_id", currentConversationId);
        const localHistory = getLocalChatHistory();
        const localChat = localHistory.find((chat) => chat.conversation_id === currentConversationId);
        if (localChat?.messages?.length > 0) {
          setMessages(localChat.messages);
          updateChatHistory(currentConversationId, localChat.messages);
        } else {
          const chatData = await fetchChatHistory(sessionId, currentConversationId);
          setMessages(chatData.messages || []);
          updateChatHistory(currentConversationId, chatData.messages);
        }
        return;
      }
      ...
      if (!currentConversationId) {
        ...
        const localHistory = getLocalChatHistory();
        const storedChat = localHistory.find((chat) => chat.conversation_id === storedConversationId);
        if (storedChat?.messages && storedConversationId) {
          setMessages(storedChat.messages);
          setConversationId(storedConversationId);
          updateChatHistory(storedConversationId, storedChat.messages);
          return;
        }
      }
    ...
    loadChat();
  }, [selectedChat, sessionId, chatHistory, setMessages, updateChatHistory]);
  ...
  async function submitNewMessage(message) {
    if (isLoading || loading) {
      console.log("Submission blocked: isLoading or loading is true");
      return;
    }
    ...
    try {
      ...
      if (!selectedChat || selectedChat.conversation_id !== currentConversationId) {
        const newChat = {
          conversation_id: currentConversationId,
        ...
        updateChatHistory(currentConversationId, newChat);
      } else {
        console.log("Updating chat history (existing chat):", updatedMessages);
        updateChatHistory(currentConversationId, updatedMessages);
      }

      // Also notify App via onSendMessage to ensure consistency
      await onSendMessage(image ? formData : messageToSend, {
        conversation_id: currentConversationId,
        session_id: sessionId,
        isImage: !!image,
      });
    } catch (error) {
      console.error("🚨 Error in submitNewMessage:", error);
      setMessages((draft) => {
  ...
  return (
    <div className="relative flex flex-col h-screen w-full bg-white font-urbanist">
      ...
      <div className="flex-grow overflow-y-auto px-4 relative z-10">
        <ChatMessages messages={messages} isLoading={isLoading || loading} />
      </div>

      <div className="sticky bottom-0 w-full bg-white shadow-md px-4 pt-0 pb-4">
        <ChatInput newMessage={newMessage} isLoading={isLoading || loading} setNewMessage={setNewMessage} submitNewMessage={submitNewMessage} />
      </div>
    </div>
  );
}
